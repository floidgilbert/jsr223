...
Our \CRANpkg{jsr223} package builds on \pkg{rJava} to provide a high-level, extended interface to the Java platform. Our primary goal is to make Java libraries easier to use from within R. We achieve this goal by (i) providing a simple interface to several programming languages that can use Java objects in natural syntax, and (ii) significantly extending data exchange capabilities between R and the Java platform. To illustrate this approach, we consider an introductory example. Some technical details are intentionally omitted at this stage.

First, we create a \pkg{jsr223} \code{ScriptEngine} object. This object dynamically executes code that accesses Java objects. In this example, we create a script engine that executes Groovy language code. Groovy is a scripting language whose syntax is very similar to the Java programming language. Hence, it is a natural choice for Java integration. The variable \code{class.path} is a character vector containing paths to the Java libraries required for this demonstration.

\begin{verbatim}
library("jsr223")
engine <- ScriptEngine$new("groovy", class.path)
\end{verbatim}
Next, we use the script engine to execute a Groovy code snippet that defines a function that returns a Java HashMap object. Notice that the syntax is very similar to Java. One obvious difference is that we can define a .
///change this to handle multiple setnences. return a list of data frames
///put the code example online in the groovy section
///does not use json. we use custom on top of \pkg{rJava}. precision. less overhead.
\begin{verbatim}
engine %@% '
  import edu.stanford.nlp.simple.*;

  def getPartsOfSpeech(String sentence) {
    s = new Sentence(sentence);
    m = new HashMap<String, Object>();
    m.put("parts.of.speech", s.posTags());
    m.put("words", s.words());
    return m;
  }
'
\end{verbatim}
Finally, we call the function from R, specifying a ///.

\begin{verbatim}
engine$invokeFunction(
  "getPartsOfSpeech",
  "The jsr223 package makes Java objects easy to use!"
)
...
Our task is to ./// This shows that we don't need to compile Java wrapper classes to implement sophisticated functionality///. This leads to faster development or prototyping.


To date, the \pkg{jsr223} package officially supports five Java-compatible languages ////:. While our primary goal is to simplify access to Java objects, we also get the benefit of ///solutions in all of these other languages. The \pkg{jsr223} package has these features.

///list the other features uncommon in these types of integration - callbacks without additional dependencies or configurations.

by providing a unified integration interface to several programming languages that target the Java platform.


from for effective Java integrations for R by making Java libraries more easily accessible and simplifying data exchange.

///In short, use Java libraries easier, exchange data easier. Also some scripting languages. Give an example.///We use the Java Scripting API \citep{jsa} as defined by the specification “JSR-223: Scripting for the Java Platform” \citep{jsr223spec} to provide a unified integration interface to several programming languages. To date, \CRANpkg{jsr223} supports five languages: Groovy, JavaScript, JRuby, Jython, and Kotlin. (JRuby and Jython are Java implementations of the Ruby and Python languages, respectively.) ///Mention libraries more clearly. All of these languages can create and use Java objects. Hence, \pkg{jsr223} enables developers to easily embed other languages in R that can, in turn, use Java classes in natural syntax. As we will show, this approach is easier to use than \pkg{rJava}'s low-level JNI interface. In addition to multi-language support for the Java platform, \pkg{jsr223} provides an intuitive callback API to access R from any of the supported languages. Finally, the \pkg{jsr223} package simplifies and extends data exchange between R and Java. In all, \pkg{jsr223} lowers the barrier to the performance benefits and cross-platform capabilities of the Java platform.
...
The third significant difference between \pkg{V8} and \pkg{jsr223} is code validation. \pkg{V8} includes an interface to validate JavaScript code without executing it. The Java Scripting API does not provide an interface for code validation, hence, \pkg{jsr223} does not provide this feature either. We have investigated other avenues to validate code, but none have been reliable across all of the supported languages. Moreover, this feature is generally not critical for most applications requiring integration. Rather, this feature is most useful for interactive code editors.
