...
In \hyperlink{use-cases}{Use cases}, we provide brief examples that demonstrate core \pkg{jsr223} functionality. \hyperlink{the-jsr223-package-implementation}{The \pkg{jsr223} package implementation} section contains a brief description of the \pkg{jsr223} software internals. The sections \hyperlink{helpful-terminology-and-concepts}{Helpful terminology and concepts}, \hyperlink{package-installation}{Package installation}, \hyperlink{script-engine-installation}{Script engine installation and instantiation}, and \hyperlink{quick-start-guide}{Quick start guide} provide the details necessary to install \pkg{jsr223} and become familiar with all of its features. If you are primarily interested in using \pkg{jsr223} with a specific programming language, jump to \hyperlink{r-with-groovy}{R with Groovy}, \hyperlink{r-with-javascript}{R with JavaScript}, \hyperlink{r-with-python}{R with Python}, \hyperlink{r-with-ruby}{R with Ruby}, or \hyperlink{r-with-kotlin}{R with Kotlin}. The section \hyperlink{software-review}{Software review} includes a discussion that puts the \pkg{jsr223} project in context with other relevant software solutions.
...
\hypertarget{use-cases}{\section{Use cases}}

This section includes two simple examples that demonstrate the core functionality of the \pkg{jsr223} package. For a complete overview of \pkg{jsr223} features, see the \hyperlink{quick-start-guide}{Quick Start Guide}.

\subsection{Using Java libraries}

This example shows how so-called ``glue'' code can be embedded in R to quickly leverage Java solutions from within R. It also demonstrates how easily \pkg{jsr223} converts Java data structures to R objects.


%///review every word and code item very closely. put this together after writing it first with groovy. changed the wording. make it very apparent that we can convert JS objects to R.
%///cite stanford
%This example demonstrates using Java libraries from R and converting Java data structures to R objects. 
Specifically, we show how to use \href{https://nlp.stanford.edu/}{Stanford's Natural Language Processing} (NLP) libraries to identify grammatical parts of speech in a text. NLP is a key component in statistical text analysis and artificial intelligence. The NLP Java libraries required for this example can be obtained from the \href{https://stanfordnlp.github.io/CoreNLP/index.html}{Stanford CoreNLP}.
...
\end{verbatim}
Now we can execute JavaScript source code. The \pkg{jsr223} interface provides several methods to execute code. In this example, we use the \code{\%@\%} operator which executes a code snippet and discards the return value, if any, instead of returning it to R. This code snippet, supplied as a character constant, imports the Stanford NLP \code{Document} class. This syntax is peculiar to the JavaScript language integration with Java. The resulting object, \code{DocumentClass} is a ///. It can be used to instantiate objects or access static methods.

\begin{verbatim}
engine %@% 'var DocumentClass = Java.type("edu.stanford.nlp.simple.Document");'
\end{verbatim}

 and defines a JavaScript function named \code{getPartsOfSpeech}. The function tags each element in a text with the corresponding grammatical part of speech. ///describe the return value being built.
...
'
\end{verbatim}
%///maybe make a comment about how concise the JS code is.
Finally, we use \code{engine\$invokeFunction} to call code{getPartsOfSpeech} from R. The \code{invokeFunction} method takes the name of the function as the first parameter and any number of function parameters. The \pkg{jsr223} package automatically converts the function's return value to R objects. In this case, it converts the list of JavaScript objects to a list of R data frames. The output is printed below. The parts of speech abbreviations are defined by the \href{https://www.ling.upenn.edu/courses/Fall_2003/ling001/penn_treebank_pos.html}{Penn Treebank Project}. %///cite
...
This example demonstrates using Java libraries from R and converting Java data structures to R objects. Specifically, we show how to use \href{https://nlp.stanford.edu/}{Stanford's Natural Language Processing} (NLP) libraries to identify grammatical parts of speech in a text. NLP is a key component in statistical text analysis and artificial intelligence. We create and use the objects in the Groovy language. Groovy syntax is very similar to the Java programming language; hence, it is a natural choice for Java integrations.

The NLP Java libraries required for this example can be obtained from the \href{https://stanfordnlp.github.io/CoreNLP/index.html}{Stanford CoreNLP}. The Groovy script engine file can be obtained by following the instructions under \hyperlink{script-engine-installation}{Script engine installation and instantiation} in this document. %///cite from https://stanfordnlp.github.io/CoreNLP/index.html#license
...
$\lambda$ &    1 & 0.955 & 1.331 & 1.571 & 1.843 & 2.422 & 0.315 & 1007 \\ 
$\lambda$ &    2 & 0.937 & 1.340 & 1.563 & 1.820 & 2.409 & 0.316 & 1068 \\ 
$\lambda$ &    3 & 0.939 & 1.321 & 1.560 & 1.808 & 2.395 & 0.316 & 1119 \\ 
$\lambda$ &    4 & 0.953 & 1.325 & 1.546 & 1.823 & 2.359 & 0.314 & 1187 \\ 
\hline
\end{tabular}
\end{table}

\hypertarget{the-jsr223-package-implementation}{\section{The \pkg{jsr223} package implementation}}
...
\hypertarget{helpful-terminology-and-concepts}{\section{Helpful terminology and concepts}}
...
\hypertarget{package-installation}{\section{Package installation}}
...
\hypertarget{script-engine-installation}{\section{Script engine installation and instantiation}}
...
\hypertarget{kotlin-script-engine-installation}{\subsection{Kotlin}}
...
\clearpage
\hypertarget{quick-start-guide}{\section{Quick start guide}}
...
\hypertarget{handling-r-vectors}{\subsection{Handling R vectors}}
...
\hypertarget{handling-r-data-frames}{\subsection{Handling R data frames}}
...
\hypertarget{data-exchange-details}{\subsection{Data exchange details}}
...
\hypertarget{string-interpolation}{\subsection{String interpolation}}
...
\clearpage
\hypertarget{r-with-groovy}{\section{R with Groovy}}
...
\clearpage
\hypertarget{r-with-javascript}{\section{R with JavaScript}}
...
\clearpage
\hypertarget{r-with-python}{\section{R with Python}}
...
\clearpage
\hypertarget{r-with-ruby}{\section{R with Ruby}}
...
\clearpage
\hypertarget{r-with-kotlin}{\section{R with Kotlin}}
...
\hypertarget{kotlin-idiosyncrasies}{\subsection{Kotlin idiosyncrasies}}
...
\hypertarget{software-review}{\section{Software review}}
...
\hypertarget{rjava-software-review}{\subsection{\pkg{rJava} software review}}
...
\hypertarget{groovy-integrations}{\subsection{Groovy integrations software review}}
...
\hypertarget{javascript-integrations}{\subsection{JavaScript integrations software review}}
...
\hypertarget{python-integrations}{\subsection{Python integrations software review}}
