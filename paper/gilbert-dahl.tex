...
The \pkg{jdx} package (and hence \pkg{jsr223}) supports copying R vectors, factors, n-dimensional arrays (including matrices), data frames, names lists, unnamed lists, nested lists (i.e. lists containing lists), and environments to the script engine (and the Java platform). Row-major and column-major ordering options are available for both arrays and data frames. R data types numeric, integer, character, raw, and logical are supported. Complex types and date/time classes are not supported. 

Java scalars, n-dimensional arrays, collections, and maps can be copied to the R environment. These structures cover practically all of the scripting data structures that you will use in the supported scripting languages. Moreover, collections and maps are ubiquitous in Java APIs; providing support for these structures gives developers instant access to a vast number of data structures available on the Java platform.
...
The \pkg{jsr223} package supports data exchange for matrices and other n-dimensional arrays. By default, n-dimensional arrays are copied in row-major order. The element corresponding to the first row, second column in a 2 x 2 R matrix is accessed as follows.
...
\begin{itemize}
\item 'row-major' -- The indexes of an n-dimensional array are ordered \newline\code{[row][column][matrix]...[n]}. The \pkg{jsr223} package defaults to 'row-major' because R uses this scheme. However, this scheme is not intuitive for Java programmers when n > 2 because Java n-dimensional arrays are constructed as large objects containing smaller objects.

\item 'column-major' -- The indexes of an n-dimensional array are ordered \newline\code{[n]...[matrix][column][row]}. This ordering scheme is natural for Java programmers: the data contained in the one-dimensional arrays represent columns of the parent matrix.

\item 'row-major-java' -- The indexes of an n-dimensional array are ordered \newline\code{[n]...[matrix][row][column]}. This provides Java programmers with a natural indexing scheme where the arrays at the one-dimensional level represent rows of the parent matrix. For matrices, 'row-major-java' and 'row-major' are equivalent.
...
In the following JavaScript example, a three-dimensional array is defined and copied to the script engine using each of the three indexing options and then printed to the console as a string. Notice 

\subsection{Handling R data frames}

R data frames can be copied to the script engine copied using either row-major or column-major order. Row-major order is the default. To change this setting, use \code{engine\$setDataFrameRowMajor(value)} where \code{value} is \code{TRUE} or \code{FALSE}. To check this setting, use \code{engine\$getDataFrameRowMajor()}.

If the row-major setting is selected (i.e., \code{engine\$setDataFrameRowMajor(TRUE)}), R data frames are converted to lists of records. Specifically, the Java object is 
