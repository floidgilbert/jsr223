...
\title{\pkg{jsr223}: A Simple R Interface to the Java Platform Using Languages Groovy, JavaScript, JRuby, Jython, Kotlin, and Scala}
...
\abstract{
The R package \pkg{jsr223} is a high-level integration for the Java platform that i.) makes Java objects easy to use from within R; ii.) provides a unified interface to integrate R with several Java-based programming languages; iii.) features extensive data exchange capabilities between R and Java, and iv.) allows R code to be executed from within the supported programming languages using a simple callback interface. This integration leverages the performance benefits and cross-platform capabilities of the Java platform and expands the computing capabilities of the R software environment.
...
The following code example is taken from \pkg{rJava}'s \href{http://www.rforge.net/rJava/}{web site}. It demonstrates \code{.jnew()} and \code{.jcall()}: the essential functions of the \pkg{rJava} API. Java objects are used to create a window with a single button, display it, and close it. If you try the example, the window may appear behind other applications. Also, the last line of code is required to close the window; it does not close if you click the close button.
...
.jcall(f, , "dispose")
\end{verbatim}
The first two lines in the code example are required to initialize \pkg{rJava}. The next lines use the \code{.jnew()} function to create Java objects: a frame and a button. The class names are denoted in JNI syntax. Of particular note in this example is the first invocation of \code{.jcall()}, the function used to call object methods. It calls the \code{add()} method of the frame object. The second parameter to \code{.jcall()} provides the explicit return type of the method; it is required for JNI to correctly identify the correct class member. Finally, the last parameter to \code{.jcall()} specifies the object to be passed to the \code{add()} method. It must be explicitly cast to the correct interface for the call to be successful.

This JavaScript snippet reproduces the \pkg{rJava} example. In comparison to the \pkg{rJava} example, the JavaScript code is more natural for most programmers to write and maintain. The fine details of method lookups and invocation are handled automatically: no explicit class names or type casts are required. This same example can be reproduced in any of the five other \pkg{jsr223}-supported programming languages.
...
\end{verbatim}
This engine object is now ready to evaluate script on demand. Script can be passed to the engine using character vectors or file paths. The sample below demonstrates embedding JavaScript code in-line with character vectors. This method is appropriate for small snippets of code. 
