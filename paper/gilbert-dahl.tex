...
\pkg{jsr223} includes a simple callback interface to access the R environment from embedded scripts, enables direct calls from R to scripted functions and methods, and many other features. In all, \pkg{jsr223} is a user-friendly, multi-language integration package that lowers the barrier to the expansive libraries and performance benefits of the Java platform.



\pkg{jsr223} uses \pkg{rJava} to leverage the Java Scripting API \citep{jsa} as defined by the specification “JSR-223: Scripting for the Java Platform” \citep{jsr223spec}. The  to provide a unified integration interface to several programming languages. 
