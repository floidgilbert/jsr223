...
Our \CRANpkg{jsr223} package builds on \pkg{rJava} to provide a high-level, extended interface to the Java platform. Our primary goal is to make Java libraries easier to use from R. We achieve this goal by (i) providing a simple interface to several programming languages that can use Java objects in natural syntax, and (ii) significantly extending data exchange capabilities between R and the Java platform. To illustrate this approach, we consider an introductory example. Several technical details are intentionally omitted to facilitate ///.
