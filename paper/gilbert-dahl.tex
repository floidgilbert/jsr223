...
By default, length-one R vectors are converted to Java scalars when passed to the script engine environment. Change this behavior with  \code{engine\$setLengthOneVectorAsArray(TRUE)}. ///I function.///reword preceding sentence. All other vectors are always converted to Java arrays. The following code passes a vector of ten random normal deviates to the script engine environment. The first element of the resulting array is returned. This syntax can be used in all of the supported scripting languages. \strong{Note:} Java arrays use zero-based indices.
