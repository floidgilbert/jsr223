...
Our \CRANpkg{jsr223} package builds on \pkg{rJava} to make Java libraries easier to use from within R. We achieve this by embedding other programming languages in R that can, in turn, use Java objects in natural syntax. As we show in the \hyperlink{rjavasoftwarereview}{\pkg{rJava} software review}, this approach is generally simpler and more intuitive than \pkg{rJava}'s low-level JNI interface. To date, \pkg{jsr223} supports embedding five programming languages that target the Java platform: Groovy, JavaScript, JRuby, Jython, and Kotlin. (JRuby and Jython are Java implementations of the Ruby and Python languages, respectively.) Besides providing simplified access to Java libraries, \pkg{jsr223} also makes countless existing solutions in these supported programming languages immediately accessible to R developers. For example, Ruby gems can be embedded in R and popular JavaScript libraries can be sourced directly from the Internet.

The \pkg{jsr223} package features extensive, configurable data exchange. R vectors, factors, n-dimensional arrays, data frames, lists, and environments can be converted to generic Java objects. Java scalars, n-dimensional arrays, maps, and collections are converted to R vectors, n-dimensional arrays, data frames, and lists. Several data exchange options are available, including row-major and column-major ordering schemes for data frames and n-dimensional arrays. Many language integrations for R provide a comparable feature set with JSON (JavaScript Object Notation) libraries. The \pkg{jsr223} package implements these features using custom Java routines to avoid the overhead of serialization and the loss of floating point precision inherent in JSON data conversion.

Other distinguishing features of the \pkg{jsr223} package include a simple callback interface to access the R environment from embedded scripts, direct calls from R to scripted functions and methods, script compiling, and string interpolation for source code. In all, \pkg{jsr223} is a user-friendly, multi-language integration package that lowers the barrier to the expansive libraries and performance benefits of the Java platform. %///what about script language features. and the other libraries?

\pkg{jsr223} uses \pkg{rJava} to leverage the Java Scripting API \citep{jsa} as defined by the specification “JSR-223: Scripting for the Java Platform” \citep{jsr223spec}. This specification (get old words) The  to provide a unified integration interface to several programming languages. 

The \pkg{jsr223} package uses \pkg{rJava} to . Perofrmance benefits by minimizing error checking and object inspection. \pkg{jsr223} is therefore lightweight and does not add much overhead compared to direct calls with \pkg{rJava}. ///maybe move to software 
Our unified, multi-language integration is made possible by the Java Scripting API \citep{jsa} as defined by the specification “JSR-223: Scripting for the Java Platform” \citep{jsr223spec}.///put this somwhere else? in software architecture? built on \pkg{rJava}. Not JSON.

\section{A first example}

%///cite stanford
This example demonstrates accessing Java libraries and converting Java objects to R. uses \href{https://nlp.stanford.edu/}{Stanford's Natural Language Processing} (NLP) libraries to identify grammatical parts of speech in a text. NLP is a key component in statistical text analysis and artificial intelligence.

%///cite Groovy.
First, we create a \pkg{jsr223} \code{ScriptEngine} object that dynamically executes source code. In this case, we use a \href{http://groovy-lang.org/}{Groovy} language script engine. Groovy syntax is very similar to the Java programming language; hence, it is a natural choice for Java integration. ///engine name/// The variable \code{class.path} is a character vector containing paths to the required Java libraries. It is supplied to the \code{ScriptEngine\$new} constructor method. The new \code{ScriptEngine} object instance is assigned to the variable \code{engine}.
...
\end{verbatim}
%///put the code example online in the groovy section
Next, we use the \code{engine} object and the \code{\%@\%} operator to execute a Groovy code snippet. The snippet imports a Stanford NLP package and defines a function named \code{getPartsOfSpeech}. The function processes English text and stores the results in a list of Java map objects. Each map object contains the results for a single sentence.
\begin{verbatim}
engine %@% '
...
'
\end{verbatim}
Finally, we use \code{engine\$invokeFunction} to call the Groovy function from R, specifying the name of the function and the text we wish to process. The \pkg{jsr223} package automatically converts the resulting Java objects to R objects. In this example, the result is a list of R data frames. Each data frame contains the words of a sentence, the corresponding parts of speech, and the beginning/ending offsets. The parts of speech abbreviations are defined by the \href{https://www.ling.upenn.edu/courses/Fall_2003/ling001/penn_treebank_pos.html}{Penn Treebank Project}. %///cite
...
The preceding example shows how simple ``glue'' code can be embedded in R to quickly and effectively leverage Java solutions. Our approach enables developers to access Java objects with less code and more natural syntax than \pkg{rJava}'s low-level JNI implementation. See \hyperlink{rjavasoftwarereview}{\pkg{rJava} software review} for a technical discussion and a direct code comparison. The example also shows how \pkg{jsr223} converts complex Java structures to R objects. Other comparable integration packages use JavaScript Object Notation (JSON) to provide a comparable feature set for data exchange. Our \pkg{jsr223} package uses custom conversion routines based on native data types that avoid the overhead of JSON serialization to strings and the consequential loss of numeric precision.

The \pkg{jsr223} package includes several options for code execution and data exchange, callbacks, etc that are not /// in this example./// see the quick start guide for a complete review of features.

\section{Document organization}

We define some needed terminology, and then we put the \pkg{jsr223} project in context with a review of relevant software. Next, there are the necessary installation instructions and a quick start guide to demonstrate major package features. The most interesting sections follow with code samples that blend R with the \pkg{jsr223} languages. All code examples related to this document are available at \href{https://github.com/floidgilbert/jsr223/tree/master/examples}{GitHub}.
///Do a sly TOC here with links to the stuff people want to read.
...
\hypertarget{javascriptintegrations}{\subsection{JavaScript integrations software review}}
...
\item \pkg{rJython}'s JSON features are provided by the \CRANpkg{rjson} package on the R side. As we describe in \hyperlink{javascriptintegrations}{JavaScript integrations}, JSON implementations incur overhead and loss of numeric precision. Furthermore, \pkg{rjson} does not handle some R structures very well. For example, n-dimensional arrays and unnamed lists are both converted to one-dimensional JSON arrays. Lastly, \pkg{rJython} converts data frames to Python dictionaries, but Python dictionaries are always returned to R as named lists. \pkg{jsr223} exhibits none of these limitations.
...
\subsection{Software review for miscellaneous integrations}

Currently, \pkg{jsr223} is the only integration for the Ruby and Kotlin languages on CRAN.

There are other programming languages that implement the JSR-223 specification, but are not officially supported by the \pkg{jsr223} project. Technically, \pkg{jsr223} will work with any of these languages. However, we may not officially support a specific implementation for a number of reasons. Our most notable exclusion is \href{https://www.scala-lang.org/}{Scala}. We decline to support the Scala JSR-223 implementation simply because it is not complete. (For a very thorough Scala integration for R, consider the \CRANpkg{rscala} package.) We also exclude languages that are no longer being developed (e.g. \href{http://www.beanshell.org/}{BeanShell}).

\subsection{Renjin and FastR software review}

%///citations
%Renjin and FastR are 
%Renjin is R on the JVM. Can use Java objects directly.
%Presumeably, so can FastR.
%
%The main advantage, \pkg{jsr223} makes Java accessible via the reference implementation. All packages available to R. Many R packages are not available to JVM because of native libraries.
%
%There are others. See this article.
%https://4dpiecharts.com/2013/10/13/fearsome-engines-part-3-which-one-should-you-use/
...
## [1] 6
\end{verbatim}
The Kotlin language is an exception to this behavior. It handles bindings through a the global object \code{jsr223Bindings} as follows. See \hyperlink{kotlinidiosyncrasies}{Kotlin idiosyncrasies} for more information.
...
///
%In all, \pkg{jsr223} significantly extends the computing capabilities of the R software environment.
%///people can use tools they know. Java is more accessible.
