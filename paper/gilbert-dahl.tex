...
Our \CRANpkg{jsr223} package builds on \pkg{rJava} to make Java libraries easier to use from within R. We achieve this by embedding other programming languages in R that can, in turn, use Java objects in natural syntax. As we show in the \hyperlink{rjavasoftwarereview}{\pkg{rJava} software review}, this approach is generally simpler and more intuitive than \pkg{rJava}'s low-level JNI interface. To date, \pkg{jsr223} supports embedding five programming languages that target the Java platform: Groovy, JavaScript, JRuby, Jython, and Kotlin. (JRuby and Jython are Java implementations of the Ruby and Python languages, respectively.) Besides providing simplified access to Java libraries, \pkg{jsr223} also makes countless existing solutions in these supported programming languages immediately available to R developers. For example, Ruby gems can be embedded in R and popular JavaScript libraries can be sourced directly from the Internet.
...
Other distinguishing features of the \pkg{jsr223} package include a simple callback interface to access the R environment from embedded scripts, direct calls from R to scripted functions and methods, script compiling, and string interpolation. In all, \pkg{jsr223} lowers the barrier to the expansive libraries and performance benefits of the Java platform and several programming languages.
...
Next, we execute a Groovy code snippet. The operator /// use the \code{engine} object and the \code{\%@\%} operator to . The snippet imports the Stanford NLP ///simple package and defines a function named \code{getPartsOfSpeech}. The function processes English text and stores the results in a list of Java map objects. Each map object contains the results for a single sentence.
\begin{verbatim}
engine %@% '
...
%///we could have imjplemented this in several ways.
...
\section{Dynamic ///something}

\section{The \pkg{jsr223} package architecture}

%///probably move to implementation/architecture
