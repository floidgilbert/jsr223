...
.jcall(f, , "dispose")
\end{verbatim}
This JavaScript snippet reproduces the \pkg{rJava} example. It can be embedded in R using \pkg{jsr223}. In comparison to the \pkg{rJava} example, the JavaScript code is more natural for most programmers to write and maintain. In addition to creating and using Java objects in script, classes can be extended and interfaces can be implemented. This same example can be reproduced in any of the five other \pkg{jsr223}-supported programming languages.
...
Java programs are compiled to Java bytecode that is executed by an instance of a Java Virtual Machine (JVM). The JVM, in turn, interprets the Java bytecode to machine code (i.e., processor-specific instructions). The JVM acts as an abstraction layer to provide a platform-independent execution environment for Java programs. JVMs are available for a wide variety of hardware and software platforms. In principle, the same Java program will run on any platform that supports a JVM. The Java paradigm contrasts with traditional compiled languages, such as C, that are compiled directly to processor-dependent machine code, and therefore must be recompiled for every targeted architecture. Often, changes in the source code are also required to support different platforms.
