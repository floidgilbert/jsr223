...
This section includes introductory examples that demonstrate the core functionality of the \pkg{jsr223} package. For a complete overview of \pkg{jsr223} features and its supported languages, see the \textit{\pkg{jsr223} User Manual} included in the package vignettes.

\subsection{Using Java libraries}

For this demonstration, we use Stanford's Core Natural Language Processing Java libraries \citep{stanfordcorenlp} to identify grammatical parts of speech in a text. Natural language processing (NLP) is a key component in statistical text analysis and artificial intelligence. This example shows how so-called ``glue'' code can be embedded in R to quickly leverage powerful Java solutions. It also demonstrates how easily \pkg{jsr223} converts Java data structures to R objects. To follow along, download the script at \url{https://github.com/floidgilbert/jsr223/tree/master/examples/JavaScript/stanford-nlp.R}.
...
Ruby gems (i.e., libraries) can also be accessed using \pkg{jsr223}. Simply provide the paths to the gems and dependencies in the class path. The \textit{\pkg{jsr223} User Manual} provides instructions and a code example.
