...
%///standardize functions with () or not.
\maketitle
...
\item Kotlin handles script engine bindings differently than the other language implementations. See \hyperlink{kotlinbindings}{Kotlin bindings} for details.
\end{enumerate}
...
The following two lines of R code are equivalent: they convert an R object to a Java object and assign the new object to a global variable \code{myValue} in the script engine's environment. This syntax is the same for all supported R data structures.
...
\hypertarget{kotlinbindings}{\subsection{Kotlin bindings}}

The Kotlin script engine handles bindings through a global map object instead of global variables. The best way to illustrate this behavior is by example. The following code creates and retrieves a binding \code{myValue} as usual.
...
The currently available Kotlin code examples use \code{bindings} instead of \code{jsr223Bindings}. However, \code{bindings} is read-only. This issue has been reported to Kotlin. Their suggested workaround is to use \code{jsr223Bindings}.

\hypertarget{kotlincallbacks}{\subsection{Kotlin callbacks to R}}

As noted in the section above, script engine bindings in Kotlin are managed through a map object named \code{jsr223Bindings} as opposed to global variables as in other script engines like JavaScript. This includes the \code{R} object that \pkg{jsr223} automatically adds to facilitate callbacks into the R environment (see \hyperlink{callbacks}{Callbacks}). However, accessing the \code{R} object via bindings is awkward because it requires an explicit cast operation to access methods. For this reason, we automatically create a statically typed reference to the \code{R} object in the global environment when the script engine is instantiated. The following code demonstrates callbacks. In short, we ensure that callbacks operate the same way in Kotlin as in other script engines despite this difference in bindings behavior.
