...
This section includes two simple examples that demonstrate the core functionality of the \pkg{jsr223} package. For more information about \pkg{jsr223} features, see the \hyperlink{quick-start-guide}{Quick Start Guide}.

\subsection{Stanford Natural Language Processor}

%///cite stanford
This example demonstrates using Java libraries from R and converting Java objects to R objects. We use \href{https://nlp.stanford.edu/}{Stanford's Natural Language Processing} (NLP) libraries to identify grammatical parts of speech in a text. NLP is a key component in statistical text analysis and artificial intelligence. %///where to download jars.

%///cite Groovy. wait, don't we reference groovy before now?
The first step is to create a \pkg{jsr223} \code{ScriptEngine} object that can dynamically execute source code. In this example, we use a \href{http://groovy-lang.org/}{Groovy} language script engine. Groovy syntax is very similar to the Java programming language; hence, it is a natural choice for Java integration. The variable \code{class.path} is a character vector containing paths to the required Java libraries. It is supplied to the \code{ScriptEngine\$new} constructor method. The \code{ScriptEngine} object instance is assigned to the variable \code{engine}.
...
Now that we have a script engine instance, we can execute a Groovy code. The \pkg{jsr223} interface provides several methods to do this. In this example, we use the \code{\%@\%} operator which executes source code and discards the return value. The snippet here imports a Stanford NLP package and defines a Groovy function named \code{getPartsOfSpeech}. It tags each word in a text with a grammatical part of speech. The return value is a list of maps. Each map contains the parts of speech and word indices for a single sentence.
...
'
\end{verbatim}
Finally, we use \code{engine\$invokeFunction} to call code{getPartsOfSpeech} from R, specifying the name of the function and the text we wish to process. The \pkg{jsr223} package automatically converts the return value to R objects; in this case, a list of data frames. Each data frame contains the words of a sentence, the corresponding parts of speech, and the beginning/ending offsets. The output is printed below. The parts of speech abbreviations are defined by the \href{https://www.ling.upenn.edu/courses/Fall_2003/ling001/penn_treebank_pos.html}{Penn Treebank Project}. %///cite
...
The preceding example shows how simple ``glue'' code can be embedded in R to quickly and effectively leverage Java solutions. The \pkg{jsr223} approach enables developers to access Java objects using natural, object-oriented syntax. The example also demonstrates how \pkg{jsr223} intuitively converts nested Java structures to R objects.
