...
Currently, the most prominent JavaScript integration for R is Jeroen Ooms' \CRANpkg{V8} package. It uses the open source \href{https://developers.google.com/v8/}{V8 JavaScript engine} used in Google's \href{https://www.google.com/chrome/browser/features.html}{Chrome browser}.

There are three primary differences between \pkg{V8} and \pkg{jsr223}. First, the JavaScript engine included with \pkg{V8} provides only essential ECMAscript functionality. This does not include file and network operations. In contrast, \pkg{jsr223} provides access to the entire JVM which includes, of course, file and network libraries as well as many others.
