...
%///how often do we say ``in contrast''?
...
Data exchange for \pkg{rJython} can be handled via \pkg{rJava} or JSON. The JSON features are provided by \CRANpkg{rjson} package on the R side and \href{https://pypi.python.org/pypi/simplejson/}{simplejson} on the Python side. As a consequence of using the simplejson library, \pkg{rJython} converts R objects to generic Python objects. In contrast, \pkg{jsr223} converts R objects to generic Java objects. 


As noted in \hyperlink{javascriptintegrations}{JavaScript integrations}, JSON implementations incur overhead and loss of numeric precision. Furthermore, \pkg{rjson} does not handle some R structures very well. For example, n-dimensional arrays and unnamed lists are both converted to one-dimensional JSON arrays. Data frames are converted to Python dictionaries, but they are returned to R as named lists. Finally, there are no configurable options for data exchange in \pkg{rJython}. \pkg{jsr223}, on the other hand, supports sophisticated R data structures, preserves numeric precision, and features many configurable options for data exchange.

The \pkg{rJython} package supports executing arbitrary Python code, but unlike \pkg{jsr223}, it will not return a value immediately. Instead, scripts must assign a value to a global Python variable which can then be fetched from R using another method. This slows down code exploration and prototyping. \pkg{rJython} supports calling Python functions and methods from R. \pkg{jsr223} also supports these features, and in addition, supports callbacks and compiling scripts. In all, \pkg{jsr223} provides a more extensive Jython integration than \pkg{rJython}.
