...
Now we can execute Groovy source code. The \pkg{jsr223} interface provides several methods to do this. In this example, we use the \code{\%@\%} operator which executes source code and discards the return value. The code snippet follows the \code{\%@\%} operator; it is simply a multi-line character constant. The code snippet imports a Stanford NLP package and defines a Groovy function named \code{getPartsOfSpeech}. The function tags each element in a text with the corresponding grammatical part of speech.  The return value is a list of maps where each map contains the parts of speech and word indices for a single sentence.
...
The preceding example shows how so-called ``glue'' code can be embedded in R to quickly and effectively leverage Java solutions. The \pkg{jsr223} integration enables developers to embed code that uses Java objects in natural syntax. The example also demonstrates how \pkg{jsr223} intuitively converts Java data structures to R objects.

\subsection{Dynamic code implementing Java interfaces ///bad title}
