...
getKotlinScriptEngineJars <- function(directory, minimum = TRUE) {
  ...
  kotlin.jars <- list.files(
    ...
    ifelse(
      minimum,
      "^kotlin-compiler\\.jar$|^kotlin-script-runtime\\.jar$|^kotlin-script-util.*\\.jar$", # kotlin-script-util.jar may have a version number, hence the .*\\.jar pattern
      "^kotlin.*\\.jar$"
    ),
    full.names = TRUE,
    all.files = TRUE,
  ...
  if (
    ...
    return(kotlin.jars)
  stop("One or more required files were not found. The following files are required: 'kotlin-compiler.jar', 'kotlin-script-runtime.jar', and 'kotlin-script-util.jar'.")
}
