...
Our analysis employs two scripts: (i) an R script to prepare the data and parameters, execute the Groovy script, and summarize the results; and (ii) a Groovy script to extend and execute the Metropolis-Hastings sampler class.

\paragraph{The Groovy script} %///change this to a paragraph style or something in the R journal.
We describe the Groovy script first. Though the script is presented here in parts, it must be executed as one complete script. Download the complete script from \href{https://github.com/floidgilbert/jsr223}{GitHub}.

To begin, we import the necessary classes. The first line imports all of the static methods of the \code{"Math"} class. The second line imports the abstract class that implements the Metropolis-Hastings sampler. The last line imports a univariate normal implementation of the proposal distribution interface \code{"MhSamplerUnivariateProposal"}.
...
This code initializes an array of normal proposal distribution objects that will be used by the sampler. Each distribution object is initialized with a variance from the array \code{proposalVariances}. The \code{proposalVariances} array values will be supplied by the R script.
\begin{verbatim}
...
Finally, we create a \code{"Sampler"} object and assign it to the variable \code{mh}. The last line runs the sampler. Notice that all of the method arguments have not been defined yet; they will be supplied by the R script.
\begin{verbatim}
...
\paragraph{The R script} First, we instantiate a Groovy script engine. We include paths to all of the required Java libraries in \code{class.path}. The first file contains the Groovy script engine; the second file contains the Metropolis-Hastings sampler, and the last file contains the \href{http://commons.apache.org/proper/commons-math/}{Apache Commons Mathematics Library}.
\begin{verbatim}
library("jsr223")
