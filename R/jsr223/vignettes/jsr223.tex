...
We describe the Groovy script first. Though the script is presented here in parts, it must be executed as one complete script. Download the complete scripts from the \pkg{jsr223} GitHub page.
...
\begin{itemize}
\item \code{import static java.lang.Math.*} imports all of the static methods of the \code{"Math"} class.
\item \code{"MhSamplerUnivariateProposal"} is the abstract class that implements the Metropolis-Hastings sampler.
\item \code{"ProposalDistributionUnivariateNormal"} is a univariate normal implementation of the proposal distribution interface \code{"MhSamplerUnivariateProposal"}.
\end{itemize}

The Groovy code below defines a class \code{\"Sampler\"} that extends \code{\"MhSamplerUnivariateProposal\"}. The constructor method takes values for the prior and computes sufficient statistics for the posterior function. These values are stored as local variables. The method 
