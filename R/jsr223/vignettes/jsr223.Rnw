...
\subsection{Document organization} %RJ - Modify
...
This section includes introductory examples that demonstrate the core functionality of the \pkg{jsr223} package. For a complete overview of \pkg{jsr223} features, see the \hyperlink{feature-documentation}{Feature documentation}. Following that section, we provide details and examples for each of the \pkg{jsr223}-supported languages. %///RJ - Refer instead to the documentation.

\subsection{Using Java libraries}

For this demonstration, we use Stanford's Core Natural Language Processing Java libraries \citep{stanfordcorenlp} to identify grammatical parts of speech in a text. Natural language processing (NLP) is a key component in statistical text analysis and artificial intelligence. This example shows how so-called ``glue'' code can be embedded in R to quickly leverage powerful Java solutions. It also demonstrates how easily \pkg{jsr223} converts Java data structures to R objects. 
...
\end{verbatim}
Now we can execute JavaScript source code. The \pkg{jsr223} interface provides several methods to do so. In this example, we use the \code{\%@\%} operator; it  executes a code snippet and discards the return value, if any. The JavaScript snippet in the following code example is supplied as a multi-line character constant. It imports the Stanford NLP \code{"Document"} class. This syntax is peculiar to the JavaScript dialect. The result, \code{DocumentClass}, is used to instantiate objects or access static methods.
...
In this example, we effectively leveraged Stanford's Core NLP library with a minimal amount of code. This same functionality can be easily replicated in any of the \pkg{jsr223}-supported programming languages.

\subsection{Extending existing Java solutions}

Many Java libraries are designed to let the developer define custom behaviors by implementing interfaces or extending classes. We illustrate one such solution in the context of a Bayesian analysis. The library at hand includes a generic, multi-threaded Metropolis-Hastings sampler. We define a custom density function for the sampler by extending (i.e., subclassing) an abstract Java class. We could have implemented this solution using anonymous classes, lambda functions, or closures. These constructs usually require less code, and they are supported within the various \pkg{jsr223}-compatible programming languages. However, we found that they did not execute as quickly as the subclassing approach.

For this example, we use the Groovy scripting language. Groovy follows the Java programming language syntax very closely; hence, it is a natural choice for Java integrations. Programmers that know Groovy will notice that our code is unnecessarily verbose. We chose to use strict Java coding conventions to make the implementation more familiar to Java programmers.
...
We choose the independent priors $\pi \sim \text{Beta}(\alpha,\beta)$ and $\lambda \sim \text{Gamma}(\theta,\kappa)$. Furthermore, we use independent Gaussian proposal densities for $\pi$ and $\lambda$.
...
The code that follows is the key element of this example; it defines the behavior of a Java class in script. Specifically, we define a class named \code{"Sampler"} that extends the abstract class \code{"MhSamplerUnivariateProposal"}. The class constructor takes prior parameter values, the data, and computes statistics used in the posterior function. The \code{logPosterior} method implements the log of the posterior function that will be called by the sampler.
...
Finally, we create a \code{"Sampler"} instance and assign it to the variable \code{mh}. The last line runs the sampler. All but one of the variables passed as method arguments have not yet been defined. They will be created in the R script.
\begin{verbatim}
...
Now we set the global variables used by the Groovy script. One way to do this is using the list-like assignment syntax as illustrated below. The first four assignments (alpha, beta, theta, and kappa) correspond to the parameter values for the prior densities; \code{data} contains the counts $y_1,\dots,y_n$; \code{proposalVariances} is an array of variances for the Gaussian proposal distributions; \code{startingValues} contains the initial ($\pi$, $\lambda$) parameter values for the MCMC chains; \code{iterations} indicates the number of MCMC iterations per chain; and \code{threads} defines the size of the parallel processing thread pool. Note that the \code{data}, \code{iterations}, and \code{threads} values are integers. The Java methods that consume them require integer types explicitly.
...
The result is stored in the variable \code{r}. It is an R unnamed list containing four named lists: one each for the four random walks. The named lists contain the chains (in the form of matrices) and runtime statistics such as acceptance ratios and elapsed time. Here we output the acceptance ratios for all four walks.
...
For the sake of demonstration, let's say that the acceptance ratios are too high. We need to widen the variances for the proposal distributions and re-run the sampler. There is no need to recompile the script; instead, just update the corresponding variable and execute the compiled script again. We output the acceptance ratios again for comparison.
...
Table \ref{tab:mcmcx} contains a summary of the results for each parameter by individual chain. The code used to summarize the results and print the table is not included here, but it can be found in the R script \file{metropolis-hastings.R} on GitHub.

\paragraph{Performance} So far we have shown that we can extend compiled Java classes from within R. But, is the runtime performance acceptable? We report some simple performance metrics here. The scripts were run on a typical laptop with an Intel i7-5500U, 2.40Ghz processor and 8GB RAM. The processor can execute four threads in parallel. We used all four threads to run four chains simultaneously. We executed the expression \code{cs\$eval()} 20 times using the \CRANpkg{microbenchmark} package \citep{microbenchmark}. The column /// in Table \ref{tab:mcmcperformance} summarizes the median run times for different numbers of iterations. We see that the increase in time is roughly linear.

The total processing time includes converting Java objects to R objects. When 1 million MCMC iterations are requested, a total of 8,000,032 values are returned to R in a structured, intuitive fashion. Taking all of this into consideration, we find the runtime performance to be acceptable.
The column /// in Table \ref{tab:mcmcperformance} summarizes the results for different numbers of iterations. We see that the increase in runtime is roughly linear. \code{cs\$eval(TRUE)}.

The /// column shows how long each  ///overhead.

It reported that the median execution time for the expression was 68 milliseconds for 10,000 MCMC iterations. To /// an idea of the \pkg{jsr223} overhead involved, 

If we increase the MCMC iterations to 100,000, the median runtime is 624 milliseconds. Increasing the iterations again to 1 million produces a median runtime of 7.7 seconds. Hence, the increase in runtime is roughly linear. 

This is only one approach to this implementation. For example, the total runtime could be reduced further by summarizing or simplifying the data on the Java side. However, summarizing data is an R language strength. We chose to transfer the unsummarized results to R in this example because we believe it represents the most typical use case.

\paragraph{Conclusion} We believe this example illustrates how \pkg{jsr223} makes advanced Java solutions more accessible to the R computing environment. Blending Groovy script with R leads to faster, and more flexible application development compared to developing Java wrapper packages.
...
$\pi$ &    2 & 0.292 & 0.461 & 0.536 & 0.607 & 0.715 & 0.223 &  805 \\
$\pi$ &    3 & 0.294 & 0.459 & 0.534 & 0.603 & 0.710 & 0.231 &  929 \\
$\pi$ &    4 & 0.311 & 0.458 & 0.532 & 0.598 & 0.714 & 0.235 &  987 \\
$\lambda$ &    1 & 0.955 & 1.331 & 1.571 & 1.843 & 2.422 & 0.266 & 1007 \\
$\lambda$ &    2 & 0.937 & 1.340 & 1.563 & 1.820 & 2.409 & 0.256 & 1068 \\
$\lambda$ &    3 & 0.939 & 1.321 & 1.560 & 1.808 & 2.395 & 0.266 & 1119 \\
$\lambda$ &    4 & 0.953 & 1.325 & 1.546 & 1.823 & 2.359 & 0.259 & 1187 \\
\hline
\end{tabular}
\label{tab:mcmcx}
\end{table}

%///probably change columns to multiline ``With return values'' and ``Without return values''
\begin{table}[h]
\centering
\caption{Benchmark timings for Metropolis-Hastings sampler and the given iterations. All times are in milliseconds. 'Run time A' are timings for \code{cs\$eval()} and 'Run time B' are timings for \code{cs\$eval(TRUE)}.///}
\begin{tabular}{rrr}
\hline
MCMC       & Median & Median \\ 
Iterations & Run time A & Run time B \\ 
\hline
10,000 & 62.503 & 39.158 \\ 
100,000 & 546.372 & 483.083 \\ 
1,000,000 & 7,429.727 & 5,205.384 \\ 
\hline
\end{tabular}
\label{tab:mcmcperformance}
\end{table}
