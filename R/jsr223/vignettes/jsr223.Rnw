...
\subsection{Implementing dynamic behavior with Java solutions}

Many well-designed libraries accept user-defined code to implement custom logic or behavior. We illustrate one such solution in the context of a Bayesian analysis. The library at hand is a generic, multi-threaded Metropolis-Hastings sampler. We define a custom density function for the sampler by extending (i.e., subclassing) a Java abstract class. We could have implemented this solution using anonymous classes, lambda functions, or closures. These approaches usually require less code, and they are supported within the various \pkg{jsr223}-compatible programming languages. However, we found that these approaches did not execute as quickly for this application.

We use the Groovy scripting language in this example. Groovy follows the Java programming language syntax very closely; hence, it is a natural choice for Java integrations. Those familiar with Groovy will notice that our code is unnecessarily verbose. We chose to the follow Java language conventions to make the code familiar to Java programmers.

The Bayesian analysis involves count data $y_1,\dots,y_n$. We believe the data to be independently and identically distributed according to a zero-inflated Poisson sampling model where $0 < \pi < 1$ and $\lambda > 0$:
...
We choose independent priors $\pi \sim \text{Beta}(\alpha,\beta)$, $\lambda \sim \text{Gamma}(\theta,\kappa)$. In our analysis, we use a Metropolis-Hastings sampler with independent Gaussian proposal densities for $\pi$ and $\lambda$.
...
\paragraph{The Groovy script} To begin, we import the necessary classes. The first line imports all of the static methods of the \code{"Math"} class. The second line imports the abstract class that implements the Metropolis-Hastings sampler. The last line imports a univariate normal proposal class. (This latter class implements the interface \code{"ProposalDistributionUnivariate"}. If we wanted to use a custom proposal distribution, we could do so by creating a class that implements this interface in script.)
...
The code that follows is the key element in this example; it implements the behavior of a Java class using Groovy script. Specifically, we define a class named \code{"Sampler"} that extends \code{"MhSamplerUnivariateProposal"}. The constructor takes values for the prior parameter and computes sufficient statistics for the posterior function. The \code{logPosterior} method implements the log posterior function that will be called by the sampler.
...
This code initializes an array of normal proposal distribution objects that will be used by the sampler. Each distribution object is initialized with a variance. The \code{proposalVariances} variable is not defined; it will be supplied by the R script.
\begin{verbatim}
...
Finally, we create a \code{"Sampler"} instance and assign it to the variable \code{mh}. The last line runs the sampler. Many of the variables passed as method arguments have not yet been defined. They will be created in the R script.
\begin{verbatim}
...
%///cite microbenchmark
So far we have shown that we can implement dynamic Java solutions from within R using a combination of compiled Java libraries and Groovy script. But, is the performance acceptable? We report some simple performance benchmarks here. The scripts were run on a laptop with an Intel i7-5500U, 2.40Ghz processor and 8GB RAM. The processor can execute four threads in parallel. We used all four threads to run four chains simultaneously. We executed the expression \code{r <- cs\$eval()} 50 times using the \CRANpkg{microbenchmark} package. It reported that the median total execution time for the expression was 68 milliseconds for 10,000 MCMC iterations. If we increase the MCMC iterations to 100,000, the median total runtime is 624 milliseconds. Increasing the iterations again to 1 million produces a median total runtime of 7.7 seconds. Hence, the performance is roughly linear. The processing time includes converting the Java results to R objects. In this case, we have an R list containing four named lists, each with several runtime statistics and a matrix of MCMC draws. When 1 million MCMC iterations are requested, 8,000,032 values are returned to R. Overall, we find this execution time acceptable, especially considering that the implementation was scripted.
...
We believe this example illustrates how \pkg{jsr223} makes advanced Java solutions more accessible to the R computing environment. Blending Groovy script with R leads to faster, and more flexible application development compared to developing Java wrapper packages.
