...
%\VignetteIndexEntry{jsr223 - R Journal (Submitted)}
...
\begin{itemize}
\item Data exchange for \pkg{rJython} can be handled via JSON or direct calls to the Jython interpreter object via \pkg{rJava}.

\item When using \pkg{rJava}, \pkg{rJython} data exchange is essentially limited to vectors and matrices.
...
Those familiar with the JSR-223 specification might be aware that there are other programming languages that implement JSR-223 interfaces. Technically, \pkg{jsr223} will work with any of these languages. However, we may not officially support a specific JSR-223 implementation for a number of reasons. Our most notable exclusion is \href{https://www.scala-lang.org/}{Scala}). We decline to support the Scala JSR-223 implementation simply because it is not complete. (For a very thorough Scala integration for R, consider the \CRANpkg{rscala} package.) We also exclude languages, such as \href{http://www.beanshell.org/}{BeanShell}, that are no longer being developed.
...
Java implentation is faster.
can use for other languages.
fits the R everything.
simple to use.
easy to learn.

\pkg{jsr223} is good. do Java stuff faster.
